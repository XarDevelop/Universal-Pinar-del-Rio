...
const Representante = () => {
  ...
  const handleAgregarDocumento = async (data) => {
    try {
      ...
      await axios.post('/api/documentos', data, {
        headers: { 'Authorization': `Bearer ${token}` }
      });
  ...
  const handleDesecharDocumento = async (nombreDocumento) => {
    try {
      const token = localStorage.getItem('token');
      // Ajusta la URL según tu API
      await axios.post('/api/documentos/desechar', { nombreDocumento }, {
        headers: { 'Authorization': `Bearer ${token}` }
      });
      cargarDocumentos(seccionActiva);
    } catch (error) {
      console.error('Error desechando documento:', error);
      setError('Error al desechar el documento');
    }
  };
  ...
  return (
    ...
    >
      <div className="representante-content">
        <PanelRepresentante 
          seccionNombre={nombreSeccionActiva}
          onAgregarDocumento={handleAgregarDocumento}
          onDesecharDocumento={handleDesecharDocumento}
        />
